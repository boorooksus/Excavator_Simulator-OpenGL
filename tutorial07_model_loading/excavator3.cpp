...
int main(void)
{
	...
	GLint objectColorID = glGetUniformLocation(programID, "objectColor");
	...
	// View matrix
	glm::vec3 viewPosition = glm::vec3(3, 5, 3);


	// Camera matrix
	// y축 위에서 xz평면을 바라본다.
	View = glm::lookAt( //4 3 -3
		glm::vec3(11, 10, -7), // Camera is at (11,10,-7), in World Space
	...
	glm::vec3 gPosition13(0.3f, 0.8f, 0.5f);
	glm::vec3 gPosition14(0.3f, 0.8f, 0.5f);

	// 핸들
	glm::vec3 gPosition2(-0.35f, 1.0f, 0.5f);
	glm::vec3 gPosition3(0.0f, 0.3f, 1.5f);
	glm::vec3 gPosition4(-0.35f, 0.2f, 1.15f);

	// bucket
	glm::vec3 gPosition5(0.0f, -0.25f, 1.645f);
	...
	do
	{
		...
		glm::mat4 RotationMatrix8 = eulerAngleYXZ(direction, 0.0f, 0.0f);
		...
		glUniform3f(objectColorID, 1.0f, 0.0f, 0.0f);
...
void computeKeyboardTranslates()
{
	...
	if (glfwGetKey(window, GLFW_KEY_D) == GLFW_PRESS) {
		...
		//float closeToCenter = -(abs(direction) - 1.3f);
		//direction += 3.14159f / 2.0f * deltaTime * speed;
		//temp = direction;
		//translateFactor += rotation * deltaTime * speed;
